...
def train_population(
    ...
    Evaluation plays every speaker against every listener: ``iid_acc`` is the
    mean over all ``n_agents**2`` pairs and ``iid_acc_min`` the worst pair.
    ``agreement`` is the fraction of objects on which two speakers send the same
    message, averaged over speaker pairs; language metrics are averaged over
    speakers.
    ...
    pairs = [new_pair() for _ in range(n_agents)]
    speakers = [p[0] for p in pairs]
    listeners = [p[1] for p in pairs]
    ...
    with open(os.path.join(out_dir, "metrics.csv"), "w", newline="") as f:
        ...
        for step, (scene, targets) in enumerate(islice(dataloader, n_steps), 1):
            ...
            lis = listeners[pairing_rng.randrange(n_agents)]
            stats = game_step(
                s.model,
                lis.model,
                s.opt,
            ...
            if lifespan and step % lifespan == 0 and step < n_steps:
                # Replace agents oldest-first: speaker 0, listener 0, speaker 1, ...
                slot = generation % (2 * n_agents)
                fresh_speaker, fresh_listener = new_pair()
                if slot % 2 == 0:
                    speakers[slot // 2] = fresh_speaker
                else:
                    listeners[slot // 2] = fresh_listener
                generation += 1
